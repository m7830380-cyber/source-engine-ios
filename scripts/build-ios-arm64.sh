...
IOS_BUNDLE_ID="${IOS_BUNDLE_ID:-com.sourceengine.csgo}"
IOS_DISPLAY_NAME="${IOS_DISPLAY_NAME:-csgo-source}"
IOS_APP_BUNDLE="${IOS_APP_BUNDLE:-csgo.app}"
IOS_IPA_NAME="${IOS_IPA_NAME:-source-engine-csgo-ios-arm64.ipa}"
IOS_IPA_FILE="${IOS_IPA_FILE:-csgo.ipa}"

export IOS_BUNDLE_ID IOS_DISPLAY_NAME IOS_APP_BUNDLE IOS_IPA_FILE

./waf configure -T release --disable-warns --ios --angle \
	--skip-sdl2-sanity-check \
	--sdl2="$SDL2_FW" \
	--protoc="$ROOT/build/host/protoc" \
	${WAF_PROJECTS:+--projects="$WAF_PROJECTS"} \
	"$@"
